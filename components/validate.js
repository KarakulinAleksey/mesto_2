const enableValidation=({formSelector, inputSelector, submitButtonSelector, inactiveButtonClass, inputErrorClass, errorClass})=>{
  ...
  const showInputError = (formElement, formInput, inputErrorClass, errorMesanger)=>{
...
//валидируем форму и выводин текст ошибки
  const isValid = (formElement, formInput)=>{
    if (!formInput.validity.valid){
      showInputError(formElement, formInput, inputErrorClass, formInput.validationMessage)
    } else {
...
//дизактивация кнопки
const toggleButtonSubmitState = (inputList, submitButton, inactiveButtonClass)=>{
...
//вешаем обработчики событий на все инпуты и кнопку отправки на форме
  const setInputsFormEventListeners = (formElement, inputSelector, submitButtonSelector) => {
